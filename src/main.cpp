...
int main(){
    ...
    Eigen::MatrixX<float> ker(3, 3);
    ker << 1/9.0, 1/9.0, 1/9.0,
           1/9.0, 1/9.0, 1/9.0,
           1/9.0, 1/9.0, 1/9.0;
    auto my_network = network::Network<float, 480, 640, 3>(
        std::vector<std::shared_ptr<layer::Layer2d<float>>>{
            // std::make_shared<layer::ConvolutionLayer<float>>(std::pair<int,int>{480, 640 * 3}, 10, ker),
        },
        std::vector<std::shared_ptr<layer::Layer<float>>>{
            ...
            std::make_shared<layer::LinearOutputLayer<float>>(2)
            // std::make_shared<layer::SoftMaxLayer<float>>(2)
            // std::make_shared<layer::SigmoidLayer<float>>(2)
        }
    );

    // auto my_network = network::Network<float, 480, 640, 3>("model.json");
